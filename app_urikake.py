...
# 【キャッシュ問題の解決】Pythonに古い一時ファイル（.pyc）を一切作らせない・読み込ませない設定
...
# app_nouhin.py と完全に同じ PySide6 の loadUiType 構成に統一
from PySide6.QtCore import Qt, QDate
from PySide6.QtWidgets import QMainWindow, QDialog, QMessageBox, QApplication, QPushButton
from PySide6.QtUiTools import loadUiType

# 共通ユーティリティのインポート
import common_utils

# 起動時の画面乗っ取りを防ぐため、メイン画面のUIファイルだけを最初にロードします
...
class UrikakeWindow(QMainWindowBase, Ui_MainWindow):
    ...
    def run_query(self):
        ...
        try:
            ...
            ORDER BY sort, code
            """
            df = pd.read_sql(query, conn, params=tuple([dt["start"], dt["end"]]))
            conn.close()
...
class SettingWindow(QMainWindow, Ui_SettingDialog):
    """出力設定変更サブ画面 クラス (無限ループ・文字消失完全解決版)"""
    ...
    def __init__(self, parent):
        ...
        # 【超安全・反転残り完全解消】
        # フォーカスが離れた瞬間に「選択（青い反転）」だけを安全に非表示にするクリーンなスタイル設定
        safe_css = "QTextEdit:!focus { selection-background-color: transparent; selection-color: #000000; }"
        if hasattr(self, "text_tokcd"): self.text_tokcd.setStyleSheet(style_css if 'style_css' in locals() else safe_css)
        if hasattr(self, "text_sort"): self.text_sort.setStyleSheet(style_css if 'style_css' in locals() else safe_css)
    ...
    def handle_tokcd_change(self):
        ...
        if not hasattr(self, "text_tokcd"): return
        txt = self.text_tokcd.toPlainText()
        
        # 改行(Enter)またはタブ空白(Tab)が撃ち込まれたか検知
        if "\n" in txt or "\t" in txt:
            # 検索とゼロ埋め処理を実行
            self.on_enter()
            
            # カーソルを安全に文字の末尾へ移動して青い反転を解除
    ...
    def handle_sort_change(self):
        ...
        if not hasattr(self, "text_sort"): return
        txt = self.text_sort.toPlainText()
        
        if "\n" in txt:
            # 【重要】プログラムによる文字書き換え時の無限ループを一時防ぐ安全装置
    ...
    def on_enter(self):
        """得意先コード入力後の検索・ゼロ埋めと画面表示処理 (無限ループ対策版)"""
        ...
        # 【重要】プログラムから文字を書き換える際、無限ループバグが起きないよう一時的にシグナルを遮断
        self.text_tokcd.blockSignals(True)
        self.text_tokcd.setPlainText(code_8)
        self.text_tokcd.blockSignals(False) # 処理が終わったら安全に再開
        ...
        if res:
            ...
            curr_sort = res if res is not None else ""
            curr_flag = res

            # 得意先名ラベルに値を代入 (100%確実に表示されます)
            self.label_tokname.setText(cust_name)

            # 表示順入力欄に値を代入 (100%確実に表示されます)
            if hasattr(self, "text_sort"):
                self.text_sort.blockSignals(True) # 書き換え時の無限ループ防止
